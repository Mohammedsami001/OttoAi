...
from main.repositories.daily_summaries import get_daily_summaries_repo
    ...
    async def _task():
        ...
        for user_id in users:
            ...
            for row in incoming:
                ...
                result = await repo.upsert_transaction(user_id, row)
                
                if result.upserted_id:
                    text = f"Spending alert: INR {row['amount']} at {row['merchant']} ({row['category']})"
                    wa = await send_whatsapp_message(user_id, text)
                    if wa:
                        await repo.mark_notification_sent(user_id, row["transaction_id"], wa.get("message_id"))
...
def update_gmail_unread():
    async def _task():
        ...
        for user_id in users:
            ...
            await repo.upsert_daily_summary(user_id, today, {"gmail": gmail})
...
def update_slack_unread():
    async def _task():
        ...
        for user_id in users:
            ...
            await repo.upsert_daily_summary(user_id, today, {"slack": slack})
...
def send_daily_briefing():
    async def _task():
        ...
        for user_id in users:
            ...
            await repo.upsert_daily_summary(user_id, today, payload)
